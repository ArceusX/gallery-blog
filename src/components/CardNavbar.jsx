...
const CardNavbar = ({ items }) => {
  ...
  // Precompute anchorIds for filtered items
  const itemsWithAnchorIds = useMemo(() => {
    const dateCounts = {};
    return filteredItems.map(item => {
      dateCounts[item.date] = (dateCounts[item.date] || 0) + 1;
      return { ...item, anchorId: `#card-${item.date}-${dateCounts[item.date]}` };
    });
  }, [filteredItems]);

  // Navigate to selected card instantly
  const handleSelect = useCallback((e) => {
    const anchorId = e.target.value;
    if (anchorId) window.location.href = anchorId;
  }, []);
    ...
    <div className="media-nav-bar">
      <div className="nav-wrapper">
        <div className="nav-section">
          <label htmlFor="date-select" className="nav-label">
            {translations.cardNavbar.goTo}
          </label>
          <select
            id="date-select"
        ...
        {/* TagFilter remains fully responsive */}
